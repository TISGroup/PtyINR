...
class PtychographyDataset(Dataset):
    def __init__(self, coordinates_x,coordinates_y, diffraction_patterns,rank):
        ...
        self.coordinates_x = torch.tensor(coordinates_x, dtype=torch.int32)#.to(device)  # Convert coordinates to tensor
        self.coordinates_y = torch.tensor(coordinates_y, dtype=torch.int32)#.to(device)
        self.diffraction_patterns = torch.tensor(diffraction_patterns, dtype=torch.float32).to(device) # Convert patterns to tensor
...
def _downsample_for_tb(img_hw: torch.Tensor, max_side=512) -> torch.Tensor:
    # img_hw: HxW on device; returns HxW, possibly downsampled
...
def _to_tb_image_single_channel(img_hw: torch.Tensor) -> torch.Tensor:
    # Normalize to [0,1] and add channel dim for TensorBoard (CHW)
...
def train_Pty_INR_SGD(rank, world_size, parameters):
    ...
    # TensorBoard writer (only on rank 0); async flush to reduce I/O stalls
    writer = None
    if rank == 0:
    ...
    ### --- Data Loading ---
    # Preprocessing code from your original function
    # Load and prepare actual_amp, x_axis, y_axis, etc.
    # -- snipped for brevity --
    # Assume actual_amp, x_axis, y_axis, obj_size are prepared
    ratio = parameters["regularized_loss_weight"]
    regularized_steps = parameters["regularized_steps"]
    show_every = parameters["show_every"]

    # Load and preprocess amplitude
    ...
    for step in range(total_steps):
        ...
        if rank == 0:
            # Scalar logging (cheap)
            writer.add_scalar("train/accumulated_loss", global_accum_loss, step)
    
            # Image logging (more expensive) every show_every steps
            if step % show_every == 0:
    
                    # Prepare images (downsample + normalize to CHW)
                    amp = torch.abs(object_act)
                    phase = torch.angle(object_act)
                    probe_amp_img = torch.abs(probe)
                    probe_phase_img = torch.angle(probe)

                    plt.imsave(os.path.join(parameters["save_path"], parameters["tag"] + "_object_amp.jpg"),amp.cpu().detach().numpy(),cmap="grey")
                    plt.imsave(os.path.join(parameters["save_path"], parameters["tag"] + "_object_phase.jpg"),phase.cpu().detach().numpy(),cmap="magma_r")
                    plt.imsave(os.path.join(parameters["save_path"], parameters["tag"] + "_probe_amp.jpg"),probe_amp_img.cpu().detach().numpy(),cmap="grey")
                    plt.imsave(os.path.join(parameters["save_path"], parameters["tag"] + "_probe_phase.jpg"),probe_phase_img.cpu().detach().numpy(),cmap="magma")
