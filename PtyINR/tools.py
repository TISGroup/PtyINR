...
def grid_return(resolution):
    # Build normalized 2D grid coordinates in [-1,1]^2 with shape (H*W, 2)
    H = resolution[0]
    W = resolution[1]
    ...
    # Stack as (x, y) then flatten
    coords_2d = torch.stack([x_coords, y_coords], dim=-1).view(-1, 2)
    return coords_2d


def get_mgrid(sidelen, dim=2):
    # dim-dimensional regular grid in [-1,1], flattened to (sidelen**dim, dim)
...
def get_loss(parameters, rank):
    # Select loss by name; print choice on rank 0 GPU
...
def get_model(parameters, obj_size, config):
    # Build object amplitude/phase networks and coordinate grid
    model_type = parameters["model_type"]
    if model_type == "siren":
        obj_net_amp = Siren(in_features=2, out_features=1, hidden_features=512,
                            hidden_layers=3, outermost_linear=True, first_omega_0=parameters["first_omega"],
                            hidden_omega_0=30)
...
def recenter_probe(probe):
    # Shift probe so its max magnitude is at the center
