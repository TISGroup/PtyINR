    ...
    for step in range(total_steps):
        ...
        if step % show_every==0:
            if imshow==True:
                ...
                axes[0].imshow(obj_amp.cpu().detach().numpy())
                axes[1].imshow(obj_phase.cpu().detach().numpy())
                axes[2].imshow(torch.abs(probe).cpu().detach().numpy())
                axes[3].imshow(torch.angle(probe).cpu().detach().numpy())
                plt.show()
            else:     
                print("Step %d, Total loss %0.6f" % (step, accumlated_loss))
...
def train_Iterative(f,parameters,probe,method):
    ...
    if method =="ePIE":
        ...
        p.engines.engine.probe_center_tol=0
